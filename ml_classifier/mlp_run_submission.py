...
def run(train_path,test_path,output_dir,net_depth=3,exclude_list=None,scale_flag=True,select_flag=False,dim_reduction=False,aug_flag=False,tta=False,**aux_config):
    ...
    for fold_num,(train_index,val_index) in enumerate(kfold.split(X)):
        ...
        fold_vote_csv['category_id'] = list(fold_result) + random.sample(label_list,1)
        fold_vote_csv.to_csv(os.path.join(output_dir, f'fold{fold_num}-vote.csv'),index=False)

        fold_prob_csv = {}
        fold_prob_csv = pd.DataFrame(fold_prob_csv)
        fold_prob_csv['sample_id'] = list(test_id) + ['d2ciob_']
        fold_prob_csv['category_id'] = list(fold_prob_result) + random.sample(label_list,1)
        fold_vote_csv.to_csv(os.path.join(output_dir, f'fold{fold_num}-prob.csv'),index=False)
    ...
    total_vote_csv['category_id'] = list(final_vote_result) + random.sample(label_list,1)
    total_vote_csv.to_csv(os.path.join(output_dir, f'fusion-vote.csv'),index=False)

    total_prob_csv = {}
    total_prob_csv = pd.DataFrame(total_prob_csv)
    total_prob_csv['sample_id'] = list(test_id) + ['d2ciob_']
    total_prob_csv['category_id'] = list(final_prob_result) + random.sample(label_list,1)
    total_prob_csv.to_csv(os.path.join(output_dir, f'fusion-prob.csv'),index=False)
...
if __name__ == '__main__':
    ...
    fea_key = hmm_key
    #HALF
    output_dir = './result/hmm_half_scale_pro/'
    ...
    # for i,act in enumerate(ACTIVATION):
        ...
        run(train_path,test_path,save_path,net_depth=3,exclude_list=exclude_list,scale_flag=True,select_flag=False,dim_reduction=False,aug_flag=False,tta=False)
    
    # QTR
    output_dir = './result/hmm_qtr_scale_pro/'
    for i in range(10):
    # for i,act in enumerate(ACTIVATION):
        save_path = os.path.join(output_dir,f'trial_{str(i+1)}')
        # total
