...
class ML_Classifier(object):
    ...
    def trainer(self,train_df,target_key,random_state=21,metric=None,k_fold=5,pred_flag=False,test_df=None,test_csv=None,save_path=None):
        ...
        predictions = le.inverse_transform(final_result)
        pred_df = pd.read_csv(test_csv)
        pred_df['sample_id'] = pred_df['key']
        pred_df['category_id'] = predictions
        pred_df[['sample_id','category_id']].to_csv(f'{save_path}/{self.clf_name}_result.csv',index=False)
        return best_model
